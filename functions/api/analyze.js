...
${comparisonImage ? `editAccepted는 추천 대상 아이템만 자연스럽게 바뀌고 얼굴·신체·포즈·배경·카메라 구도·추천 대상이 아닌 다른 의상이 원본과 실질적으로 동일할 때만 true로 설정하세요. 전체 인물을 다시 그렸거나 비대상 영역이 달라졌다면 false여야 합니다.
...
5. 워스트 매치(worstMatches): 개선 포인트 제공은 이 서비스의 핵심이므로 어떤 코디라도 반드시 최소 1개, 최대 3개를 반환하세요. 완성도가 매우 높은 코디라도 배열을 비워서는 안 되며, 신발·패션 액세서리·소재·색상 중 가장 효과적인 "한 끗 업그레이드"를 최소 1개 제안하세요. 사진 전체의 상의·하의·신발·패션 액세서리를 각각 검토하고, 실제로 개선 가치가 있는 서로 다른 지점을 중요한 순서대로 배열에 담으세요. 서로 다른 개선 지점이 2개 이상 보이면 절대로 1개에서 멈추지 말고 반드시 2~3개를 반환하세요. 같은 아이템을 표현만 바꿔 중복하지 마세요.
