...
function startScanningSequence() {
  ...
  callAnalyzeAPI(state.currentOotdImage, state.selectedTpo)
    ...
    .catch(err => {
      console.warn("AI analysis failed; using temporary local result.", err);
      state.apiData = null;
      apiFailed = true;
      apiError = err;
      apiResolved = true;
  ...
  setTimeout(() => {
    ...
    const checkCompletion = setInterval(() => {
      if (apiResolved) {
        ...
        if (apiFailed) {
          showToast(aiErrorMessage(apiError, ' 임시 결과를 보여드릴게요.'));
        } else {
          showToast("AI 분석 완료! 🎯");
        }
